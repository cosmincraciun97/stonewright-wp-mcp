...
| `STONEWRIGHT_MCP_TOOL_PROFILE` | Optional compact proxied tool surface. Defaults to `essential` for fast startup with Stonewright fast-path tools; use `low-tools` for strict tool-cap clients; aliases like `antigravity`, `gemini`, `elementor`, `design`, `acf`, `cpt-ui`, `fse`, and `wp cli` normalize to canonical compact profiles; set `full` to proxy every WordPress MCP tool |
...
other strict tool-cap clients. It keeps the proxied WordPress surface near the
minimum composite build path while direct WP-CLI recovery tools remain local.
