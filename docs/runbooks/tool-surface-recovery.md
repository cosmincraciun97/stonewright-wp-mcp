...
| Reconnect reports `Tool <name> is already registered` | The running companion predates reconnect-safe tool-handle reuse. | Upgrade the companion, restart the MCP client once, then call `stonewright-reconnect`. Current companions reuse existing Plugin/Direct tool and prompt handles. |
