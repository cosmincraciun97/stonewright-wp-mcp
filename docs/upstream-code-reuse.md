...
|---|---|---|---|---|---|---|
| `elementor/elementor` `modules/mcp/abilities/manage-default-styles-ability.php` | `3afafe33b7499b4e8fcb4c684e55111721bb0c96` | Provider Router contract fixture and certification checks | Behavioral reference; public schema semantics only, no implementation copied | GPL-3.0-or-later | Records the exact input/output object schemas, required fields, update/delete/tag/CSS/mode semantics, `idempotent=false`, `CLASS_TYPE=class`, and runtime `MAX_BATCH_SIZE` ceiling of 20; Stonewright keeps upstream writes disabled | `plugin/tests/Unit/Elementor/Provider/ProviderRouterTest.php`; authentic fixture plus annotation, schema, semantic, constant, limit, ownership, trust, and safety-closure mismatch tests |
