...
export async function themeActivate(
	...
	if (input.confirm !== true) {
		throw new Error('confirm:true is required for this tool (stonewright-theme-activate)');
	}
...
export async function customCss(
	ctx: DirectToolContext,
	input: { action: 'get' | 'update'; css?: string | undefined; confirm?: boolean | undefined },
) {
	...
	if (input.action === 'get') {
		...
		return { supported: true, action: 'get', settings: { [found]: settings[found] } };
	}
	assertWriteAllowed({
		mode: ctx.writeMode,
		destructive: true,
	...
	try {
		const updated = await ctx.client.post<Record<string, unknown>>('/wp/v2/settings', {
