...
export type GutenbergBlockSpec =
	...
	| { type: 'image'; url: string; alt?: string; id?: number; width?: number; height?: number }
	| { type: 'columns'; columns?: number; children?: GutenbergBlockSpec[] }
	| { type: 'group'; layout?: string; children?: GutenbergBlockSpec[] }
	| { type: 'buttons'; children?: Array<{ type: 'button'; text: string; url?: string; style?: string }> }
...
function composeBlock(spec: GutenbergBlockSpec): string {
	switch (spec.type) {
		...
		case 'columns': {
			...
			const cols = Math.min(Math.max(spec.columns ?? Math.max((spec.children ?? []).length, 2), 1), 6);
			const columnBlocks = (spec.children ?? []).length > 0
				...
				: Array.from({ length: cols }, () => '<!-- wp:column -->\n<div class="wp-block-column"></div>\n<!-- /wp:column -->').join('\n\n');
		...
		case 'buttons': {
			const buttons = (spec.children ?? []).map((btn) => composeBlock(btn)).join('\n\n');
		...
		case 'list': {
			...
			const items = spec.items.map((item) => `<!-- wp:list-item -->\n<li>${escapeHtml(item)}</li>\n<!-- /wp:list-item -->`).join('\n');
...
export function gutenbergCompose(
	...
	return {
		...
		usage: 'Pass markup as content to stonewright-content-create-page, stonewright-content-create-post, or stonewright-content-update.',
	};
}
