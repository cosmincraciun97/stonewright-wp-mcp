...
export async function oembed(
	ctx: DirectToolContext,
	input: { url: string; maxwidth?: number | undefined; proxy?: boolean | undefined },
) {
...
export async function urlDetails(ctx: DirectToolContext, input: { url: string }) {
	assertToolEnabled(ctx.site, 'stonewright-url-details');
	try {
		return await ctx.client.get('/wp-block-editor/v1/url-details', { query: { url: input.url } });
	} catch (err) {
...
export async function blockDirectorySearch(
	ctx: DirectToolContext,
	input: { term: string; page?: number | undefined; per_page?: number | undefined },
) {
	assertToolEnabled(ctx.site, 'stonewright-block-directory-search');
	const perPage = Math.min(Math.max(input.per_page ?? 20, 1), 50);
	const page = Math.max(input.page ?? 1, 1);
	const items = await ctx.client.get<unknown[]>('/wp/v2/block-directory/search', {
...
export async function patternDirectorySearch(
	...
	const perPage = Math.min(Math.max(input.per_page ?? 20, 1), 50);
	const items = await ctx.client.get<Array<Record<string, unknown>>>('/wp/v2/pattern-directory/patterns', {
		query: {
			search: input.search,
