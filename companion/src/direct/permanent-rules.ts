...
export const CANONICAL_OPERATING_RULES: Readonly<Record<string, string>> = {
	...
	elementor_write_closure:
		'Elementor write closure: plan against the live schema, consolidate changes for one post into one dry-run batch, and never run parallel Elementor writes. After apply, invalidate only the post HTML/object cache, then call stonewright-elementor-post-write-verify without regenerate_css; it uses a guarded target-post CSS transaction before desktop, tablet, and mobile browser verification. For boxed containers measure both the outer element and .e-con-inner. A meta readback alone is not completion.',
};
