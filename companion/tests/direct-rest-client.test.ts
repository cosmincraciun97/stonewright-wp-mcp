...
describe('WpRestClient', () => {
	...
	it('maps 401 to a credential hint', async () => {
		...
		await expect(client.get('/wp/v2/pages')).rejects.toMatchObject({
			status: 401,
			hint: expect.stringMatching(/Application Password/i),
		} satisfies Partial<WpRestError>);
	});
	...
	it('maps 403 to a capability hint', async () => {
		...
		const client = new WpRestClient(site, { fetchImpl });
		await expect(client.post('/wp/v2/pages', { body: { title: 'X' } })).rejects.toMatchObject({
			status: 403,
			hint: expect.stringMatching(/capability/i),
		});
	});
	...
	it('maps 404 on wp/v2 to a permalinks hint', async () => {
		...
		await expect(client.get('/wp/v2/pages')).rejects.toMatchObject({
			status: 404,
			hint: expect.stringMatching(/permalink/i),
		});
	});
