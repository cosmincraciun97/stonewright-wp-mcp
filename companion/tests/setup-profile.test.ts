...
describe('buildSetupProfile', () => {
	it('builds a copy-paste npx MCP config for local macOS installs without requiring a manual app password', () => {
		...
		expect(profile.mcp_server.args).toEqual([
			'-y',
			'https://github.com/cosmincraciun97/stonewright-wp-mcp/releases/download/v1.0.0-alpha.28/stonewright-companion-1.0.0-alpha.28.tgz',
		]);
	...
	it('preserves Windows paths and recommends cmd-safe env-only config', () => {
		...
		expect(profile.install_command).toBe('npm install -g https://github.com/cosmincraciun97/stonewright-wp-mcp/releases/download/v1.0.0-alpha.28/stonewright-companion-1.0.0-alpha.28.tgz');
