...
4. Call `stonewright-elementor-data-update` (automatic backup under `~/.stonewright/backups/`), then open the page URL and verify. Direct mode invalidates only the post HTML/object cache, preserves CSS metadata, and never runs a CSS flush; use Plugin mode for guarded target-post CSS closure.
