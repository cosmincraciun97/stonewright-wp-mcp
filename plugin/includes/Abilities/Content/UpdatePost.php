 ...
 */
final class UpdatePost extends AbilityKernel {

	public function name(): string {
		return 'stonewright/content-update-post';
	}

	public function label(): string {
		return __( 'Update post', 'stonewright' );
	}

	public function description(): string {
		return __( 'Updates an existing post. Snapshots state before writing.', 'stonewright' );
	}
	...
	public function permission_callback( array $args ): bool|\WP_Error {
		...
		if ( $id <= 0 ) {
			return new \WP_Error(
				...
				__( 'Invalid post ID.', 'stonewright' ),
				[ 'status' => 400 ]
			);
		}
		...
		if ( ! Permissions::edit_post( $id ) ) {
			return new \WP_Error(
				...
				__( 'Insufficient capability to edit this post.', 'stonewright' ),
				[ 'status' => 403 ]
			);
		}
	...
	public function execute( array $args ): array|\WP_Error {
		return $this->audit(
			...
			function ( array $args ) {
				...
				$payload = [ 'ID' => $id ];
				foreach ( [ 'title' => 'post_title', 'content' => 'post_content', 'excerpt' => 'post_excerpt', 'status' => 'post_status' ] as $arg => $field ) {
					if ( isset( $args[ $arg ] ) ) {
						if ( 'content' === $arg ) {
							$payload[ $field ] = wp_kses_post( (string) $args[ $arg ] );
						} elseif ( 'status' === $arg ) {
							$payload[ $field ] = (string) $args[ $arg ];
						} else {
							$payload[ $field ] = sanitize_text_field( (string) $args[ $arg ] );
						}
					}
				}
				...
				return [ 'id' => $id, 'snapshot_id' => $snapshot_id, 'meta_skipped' => $meta_skipped ];
			}
		);
	}
}
