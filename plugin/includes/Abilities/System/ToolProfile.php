...
final class ToolProfile extends AbilityKernel {
	...
	public static function profile_tools( string $profile, string $task = '', string $surface = 'unknown', string $intent = 'unknown' ): array {
		...
		$rest = match ( $profile ) {
			...
			'elementor-design' => [
				...
				'stonewright/elementor-v3-kit-batch-mutate',
	...
	private static function why( string $name ): string {
		return match ( $name ) {
			...
			'stonewright/security-audit-reconcile' => 'Preview count-only legacy audit reconciliation, then apply it explicitly with production-safe confirmation and verification.',
			'stonewright/security-runtime-data-purge' => 'Preview count-only audit, memory, incident, and recurring-error state; purge only an exactly reviewed state and retain one cleanup receipt.',
			'stonewright/incident-repair-record' => 'Resolve an incident and promote one reusable lesson only after persisted failure and verifier events prove the repair.',
			...
			'stonewright/design-quality-check' => 'Turn measured browser evidence for a rendered Elementor or Gutenberg page into coverage, findings, and repair hints; store the report with persist.',
				'stonewright/oauth-header-diagnostic' => 'Separate OAuth MCP header delivery from ordinary REST and Application Password authentication without returning header values.',
			...
			'stonewright/elementor-v3-kit-batch-mutate' => 'Transactionally update kit container width, colors, and typography with dry-run, snapshot, readback, and preserved unknown settings.',
			...
			'stonewright/wc-status' => 'Detect WooCommerce, HPOS, product types, Woo blocks, and integration support levels before catalog work.',
			'stonewright/wc-product-save' => 'Dry-run, then create or update an allowlisted native WooCommerce product and verify readback.',
			'stonewright/wc-variation-save' => 'Dry-run, then create or update a variation under a verified variable parent.',
			'stonewright/wc-catalog-audit' => 'Run a bounded catalog quality audit for SKU, price, image, category, stock, and variation gaps.',
