...
final class McpUsePolicy {
	...
	public static function canonical_operating_rules(): array {
		return [
			...
			'elementor_write_closure'       => 'Elementor write closure: plan against the live schema, consolidate changes for one post into one dry-run batch, and never run parallel Elementor writes. After apply, invalidate only the post HTML/object cache, call stonewright-elementor-css-regenerate when generated CSS must be rebuilt, then call stonewright-elementor-post-write-verify for observation-only render checks before desktop, tablet, and mobile browser verification. For boxed containers measure both the outer element and .e-con-inner. A meta readback alone is not completion.',
		];
	}
