...
final class AgentInstructions {
	...
	public static function default(): string {
		$parts = [
			...
			'- For browser testing, screenshots, or visual inspection, use an external Playwright MCP. If the MCP client has no browser tool available, install/connect the external Playwright MCP with command `npx -y @playwright/mcp@latest --caps=testing,vision,devtools` before implementation. Stonewright itself does not expose browser or screenshot tools.',
			'- For visual implementation tasks, do not write blind. Extract measured tokens from the reference screenshot first: canvas size, section bounds, max widths, colors, typography, spacing, and asset crop bounds. Then build, screenshot the live page at the same viewport, list visible deltas, and iterate.',
			...
			'- If stonewright/wp-cli-status returns available=false, use direct companion MCP tools companion_wp_cli_status / companion_wp_cli_discover / companion_wp_cli_run when the MCP client exposes them. If those are not available, use normal Stonewright REST abilities; do not create sandbox files or arbitrary REST workarounds for basic page/template/meta writes.',
