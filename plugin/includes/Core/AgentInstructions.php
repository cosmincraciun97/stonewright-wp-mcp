...
final class AgentInstructions {
	...
	public static function server_bootstrap_summary(): string {
		$parts = [
			...
			'- Elementor: typed writes only; unique V3 ids, no raw/double-encoded _elementor_data, stripped settings, silent widgetType remaps, or full-tree rewrites. Use surgical batch-mutate.',
			'- If elementor-v3-batch-mutate is missing, refresh the client surface; never use php-execute or raw meta instead.',
			'- Design: read the direction brief, normalize DesignEvidence, plan natively, then write live schemas in small verified batches.',
			'- Blueprints: ask for the builder. Elementor fails if missing; Gutenberg/blocks/FSE uses engine=gutenberg; auto reports engine_used.',
			'- Repeated rows: prefer content-bulk-upsert-posts or wp-cli-batch-run.',
			'- Woo: wc-status first; use typed product, variation, term, attribute, and audit tools. Direct mode reads only.',
			'- Woo layouts require live Elementor widget or registered woocommerce/* block schemas; never invent controls or attributes.',
			'- WP-CLI stays tokenized. PHP snippets use stonewright-php-execute, never WP-CLI eval, shell, package, --exec, or --require.',
	...
	public static function default( bool $include_visual = true ): string {
		$parts = [
			...
			'- Elementor CSS safety: Never pass regenerate_css to stonewright-elementor-post-write-verify. The verifier has no global-regeneration switch; it updates only post CSS inside a post-only guarded transaction, counts direct CSS assets, probes any existing post/custom-frontend.min.css/custom-pro-widget-nav-menu.min.css URLs before and after, and restores the asset snapshot on collateral change.',
			...
			'- If the user corrects the agent or a repeatable mistake is detected, call stonewright/learning-record (MCP: stonewright-learning-record). Success requires verified:true after readback; report memory_id and scope. Never claim memory was stored without that receipt. Any generated skill remains a disabled draft.',
			'- Before browser automation on a site/client, ask once whether to use the external Playwright MCP (recommended default), another already-connected browser provider, or no browser automation. Do not scan private config or the client tool surface without permission. If the selected provider is not visible, ask once for scan permission; if absent, ask separately for explicit install/configuration permission before showing or running setup. Never install, reconfigure, or launch a browser provider silently. After explicit permission, the Playwright setup command is `npx -y @playwright/mcp@latest --caps=testing,vision,devtools`; restart the AI client and stop before the first visual write until the selected browser tool is visible. Browser automation never bypasses Stonewright custom-code dry-run, approval, backup, permission, or confirmation gates.',
			...
			'- When a task establishes a new visual direction (design_scope new_identity, replacement, or rebrand), stop after the first section: build only that section, render and capture desktop, tablet, and mobile, run stonewright/design-quality-check on the measured evidence, show the user coverage, findings, and screenshots, get explicit approval, record it with stonewright/design-checkpoint-record, then pass the returned checkpoint_token to every later section write. Maintenance scopes (preserve, repair, content_only, responsive_fix) are not gated and continue without interruption.',
			...
			'- Never guess WordPress/Elementor/Gutenberg schemas — read first, research official docs when unknown, verify after writes. Repair open incident actions from task-start before new work; use typed repair, verify, then record only a generic verified recipe.',
			'- For comment moderation, user management, application passwords, widgets, allowlisted settings, theme activation, plugin lifecycle, revision restore, granular site-health tests, search/oEmbed, and WooCommerce catalog work, use dedicated Stonewright abilities instead of raw REST or shell workarounds.',
			...
			'- For WooCommerce, call stonewright/wc-status first. Prefer typed wc-product, wc-variation, wc-term, wc-attribute, and wc-catalog-audit abilities; use tokenized wp wc only for a missing typed operation or a long batch.',
			'- WooCommerce product and variation writes dry-run by default, require the task context token, use native Woo objects, and read back the saved object. Deletes preview first and trash by default; permanent deletes require force=true.',
			'- For Elementor storefront layouts, read live Elementor Woo widget schemas and use schema-first V3 batch mutations. For Gutenberg/FSE, discover registered woocommerce/* blocks, read each block schema, and use native block/template abilities. Never invent widget controls, block attributes, or hidden Woo meta.',
			...
			'- Before a design-derived write, read stonewright/design-direction-brief once, normalize observations into DesignEvidence 1.0, and call stonewright/design-native-plan. Reuse the compact brief across section batches. Do not pass raw Figma trees or AI-generated raw Elementor settings.',
			...
			'- Custom PHP/CSS/JS/HTML requires the approval-gated typed tool. Run dry_run, show the user approval_url, exact path, byte counts, and a short summary, then stop. Never open or submit the approval page unless the user explicitly asks. After the human returns custom_code_grant, apply only the exact approved candidate to the intended theme-file or Customizer CSS surface.',
