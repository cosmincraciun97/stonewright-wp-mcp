...
final class ConnectClientConfig {
	...
	/**
	 * Companion --profile for the agent setup prompt.
	 * Default essential-static. full only when the operator surface is full
	 * and the client can accept the full catalog.
	 */
	private static function prompt_companion_profile( string $client_slug, string $wp_surface ): string {
		$client_slug = self::resolve_client_slug( sanitize_key( $client_slug ) );
		$strict_cap  = in_array( $client_slug, [ 'antigravity', 'antigravity-cli', 'gemini-cli' ], true );
		if ( 'full' === $wp_surface && ! $strict_cap ) {
			return 'full';
		}
